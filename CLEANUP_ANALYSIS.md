...
`comprehensive_cleanup.py` provides a complete system reset for fresh project starts. It removes all user data, annotations, images, and configurations while optionally preserving trained model checkpoints.
...
1. Choose whether to clear model checkpoints too
2. Confirm before any data is deleted
...
5. **Clear model data** — always clears `models/brush_cache/`; optionally clears `models/checkpoints/` and `models/saved_models/` if confirmed
...
- Model checkpoints in `models/checkpoints/` (unless you answer `y` to the prompt)
