...
def save_model_config(model_filename, class_names):
...
print("=" * 60)
print("🔍 GCS CONFIGURATION DEBUG INFO:")
print("=" * 60)
...
update_progress(0, 100, "initializing", "Loading class configuration...")
class_names = load_class_configuration()
print(f"✅ Classes: {class_names}")
update_progress(0, 100, "initializing", f"Classes: {class_names}")
...
print("🔗 Connecting to GCS and loading dataset...")
update_progress(0, 100, "initializing", "Connecting to GCS and loading dataset...")
...
update_progress(0, 100, "running", "Starting training...")
...
try:
    for epoch in range(epochs):
        epoch_msg = f"🔄 Epoch {epoch+1}/{epochs} - Memory: {get_memory_usage():.1f} MB"
        print(epoch_msg)
        ...
        epoch_summary = f"Epoch {epoch+1}/{epochs} - Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}"
        print(epoch_summary)
    ...
    try:
        ...
        model_filename = f"final_model_{timestamp}.pth"
        torch.save(model.state_dict(), f"models/checkpoints/{model_filename}")
        save_model_config(model_filename, class_names)  # Save config alongside model
        final_save_msg = f"✅ Final model saved as {model_filename}!"
    ...
    completion_msg = f"\n✅ Training complete! Final model saved as {model_filename} in models/checkpoints/"
    ...
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_filename = f"interrupted_model_{timestamp}.pth"
        torch.save(model.state_dict(), f"models/checkpoints/{model_filename}")
        save_model_config(model_filename, class_names)  # Save config alongside model
        save_msg = f"✅ Interrupted model saved as {model_filename}!"
        print(save_msg)
    ...
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_filename = f"error_model_{timestamp}.pth"
        torch.save(model.state_dict(), f"models/checkpoints/{model_filename}")
        save_model_config(model_filename, class_names)  # Save config alongside model
        save_msg = f"✅ Error model saved as {model_filename}!"
        print(save_msg)
